...
def extract_grid(location):
    ...
    utm_buildings = buildings.to_crs(crs="EPSG:32632")  # UTM zone for Denmark (zone 32N)
    minx, miny, maxx, maxy = utm_buildings.total_bounds
    grid = round(maxx - minx),round(maxy - miny)
    return grid

X,Y = extract_grid("Technical University of Denmark, Lyngby, Denmark")
#X=1021,Y=1428

#The random movement model:

class Simulation_lean:
  def __init__(self,N,T,s,r,cumulative=True):
    self.N = N
    self.T = T
    self.r = r
    self.s = s
...
def precompute_environment(location, frac_homes, frac_work):
    ...
    buildings = ox.features_from_place(location, tags={'building': True})
    utm = buildings.to_crs(crs="EPSG:32632")
