...
RUN_TESTS = ["RM_WEEKDAY"]

from Preprocessing import (
    collect_stats_vector,
    stats_vector_whole_cumulative_graph,
...
if __name__ == "__main__":
    ...
    if "RM_MONTH" in RUN_TESTS:
        run_abc("RM_MONTH", simulate_random_month, prior_rm, stats_vector_whole_cumulative_graph)

    if "RM_WEEKDAY" in RUN_TESTS:
        run_abc("RM_WEEKDAY", simulate_random_weekday, prior_rm, stats_vector_weekday_avg_macro)

    if "SM_MONTH" in RUN_TESTS:
        run_abc("SM_MONTH", simulate_scheduled_month, prior_sm_month, stats_vector_whole_cumulative_graph_allstats)

    if "SM_WEEKDAY" in RUN_TESTS:
        run_abc("SM_WEEKDAY", simulate_scheduled_weekday, prior_sm_weekday, stats_vector_weekday_avg_allstats)